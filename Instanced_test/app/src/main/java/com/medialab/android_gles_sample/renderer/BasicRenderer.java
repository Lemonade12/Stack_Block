...
public class BasicRenderer {
	...
	public static int V_ATTRIB_OFFSET = 4;//여기
	...
	int[] mVboIndices = {0};
	int[] quadVAO = {0};
	int[] instanceVBO = {0};//여기
	int index = 0;
	float offset = 4.0f;
	//Vector3f[] translations2 = new Vector3f[100];//여기
...
/// Sets vertex shader and fragment shader for rendering
	...
	public void RenderFrame() {
		...
		GLES30.glClear(GLES30.GL_DEPTH_BUFFER_BIT | GLES30.GL_COLOR_BUFFER_BIT);//화면 초기화 지워버린다
	...
	void CreateTexBuffer(TexData newTex, int[] target) {
		...
		TexBuffer(GLES30.GL_TEXTURE_2D, newTex);
	...
	void TexBuffer(int type, TexData newTex) {
		...
		GLES30.glTexImage2D(type, 0,
				newTex.format,
				newTex.width, newTex.height, 0,
				newTex.format,
				GLES30.GL_UNSIGNED_BYTE, newTex.pixels);
	...
	void CreateVbo() {
		Log.i(TAG, "CreateVbo\n");
		for(int x=0; x<100; x++)//여기부터
		{
			Vector3f translation = new Vector3f();
			translation.add(0,offset*x,0);
			translations.put(translation.get(x));
		}
		GLES30.glGenBuffers(1,instanceVBO,0);
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, instanceVBO[0]);
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER, 3 *100, translations,GLES30.GL_STATIC_DRAW);
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER,0);
		GLES30.glGenVertexArrays(1, quadVAO,0);
		GLES30.glBindVertexArray(quadVAO[0]);//여기까지

		GLES30.glGenBuffers(1, mVboVertices, 0);
		GLES30.glGenBuffers(1, mVboIndices, 0);

		mVertexData.position(0);
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, mVboVertices[0]);
		GLES30.glBufferData(GLES30.GL_ARRAY_BUFFER,
				mVertexSize * 4,
				mVertexData, GLES30.GL_STATIC_DRAW);

		mIndices.position(0);
		GLES30.glBindBuffer(GLES30.GL_ELEMENT_ARRAY_BUFFER, mVboIndices[0]);
		...
		int offset = 0;
		GLES30.glEnableVertexAttribArray(V_ATTRIB_POSITION);
		GLES30.glVertexAttribPointer(V_ATTRIB_POSITION, 3, GLES30.GL_FLOAT, false, stride, offset);

		offset += 4 * 3;
		GLES30.glEnableVertexAttribArray(V_ATTRIB_NORMAL);
		GLES30.glVertexAttribPointer(V_ATTRIB_NORMAL, 3, GLES30.GL_FLOAT, false, stride, offset);

		GLES30.glEnableVertexAttribArray(V_ATTRIB_OFFSET);//여기부터
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER, instanceVBO[0]);
		GLES30.glVertexAttribPointer(V_ATTRIB_OFFSET, 3, GLES30.GL_FLOAT, false, 0, 0);//stride 0 띄어넘을것없으니까
		GLES30.glBindBuffer(GLES30.GL_ARRAY_BUFFER,0);
		GLES30.glVertexAttribDivisor(V_ATTRIB_OFFSET,1);
		GLES30.glBindVertexArray(0);//여기까지

		// If renderer has texture, we should enable vertex attribute for texCoord
		if (mHasTexture) {
			offset += 4 * 3;
			GLES30.glEnableVertexAttribArray(V_ATTRIB_TEX);
			GLES30.glVertexAttribPointer(V_ATTRIB_TEX, 2, GLES30.GL_FLOAT, false, stride, offset);
...
//		Matrix.setIdentityM(worldMat, 0);
	...
	void Draw() {
		//GLES30.glDrawElements(GLES30.GL_TRIANGLES, mIndexSize, GLES30.GL_UNSIGNED_SHORT, 0);//count,type,offset?
	    GLES30.glBindVertexArray(quadVAO[0]);
		GLES30.glDrawArraysInstanced(GLES30.GL_TRIANGLES,0,mIndexSize,100);
	 ...
	 *****************************/
	void LogInfo() {
		BasicUtils.PrintGLstring("Version", GLES30.GL_VERSION);
		BasicUtils.PrintGLstring("Vendor", GLES30.GL_VENDOR);
		BasicUtils.PrintGLstring("Renderer", GLES30.GL_RENDERER);
		BasicUtils.PrintGLstring("Extensions", GLES30.GL_EXTENSIONS);
		BasicUtils.PrintGLstring("GLSLversion", GLES30.GL_SHADING_LANGUAGE_VERSION);
	}
