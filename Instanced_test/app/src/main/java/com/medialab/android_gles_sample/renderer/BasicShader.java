...
public class BasicShader {
	...
	int LinkShaders(int... shaders)
	{
		...
		if (program != 0)
		{
			...
			{
				GLES30.glAttachShader(program, each_shader);
				BasicUtils.CheckGLerror("glAttachShader");
			}

			// GLES30 doesn't need this
		//	GLES30.glBindAttribLocation(program, V_ATTRIB_POSITION, "position");
		//	GLES30.glBindAttribLocation(program, V_ATTRIB_NORMAL, "normal");
		//	GLES30.glBindAttribLocation(program, V_ATTRIB_TEX, "texCoord");
		//	GLES30.glBindAttribLocation(program, V_ATTRIB_TANGENT, "tangent");

			GLES30.glLinkProgram(program);
			BasicUtils.CheckGLerror("glLinkProgram");

			int[] linkStatus = {GLES30.GL_FALSE};
	...
	int GetUniformLocation(String name)
	{
		...
		{
			mUniformLocations.put(name, GLES30.glGetUniformLocation(mProgram, name));
		}
	...
	void SetUniform(String name, float x, float y, float z)
	{
		...
		BasicUtils.CheckGLerror("GLES30.glUniform3f");
	}
	...
	{
		int loc = GetUniformLocation(name);
		GLES30.glUniform4f(loc, v.x, v.y, v.z, v.w);
		BasicUtils.CheckGLerror("GLES30.glUniform4f");
	}
	*/
	...
	{
		int loc = GetUniformLocation(name);
		GLES30.glUniform2f(loc, v.x, v.y);
		BasicUtils.CheckGLerror("GLES30.glUniform2f");
	}
	*/
	...
	{
		int loc = GetUniformLocation(name);
		GLES30.glUniformMatrix4fv(loc, 1, false, m, 0);
		BasicUtils.CheckGLerror("GLES30.glUniformMatrix4fv");
	}
	...
	{
		int loc = GetUniformLocation(name);
		GLES30.glUniformMatrix3fv(loc, 1, GL_FALSE, value_ptr(m));
		BasicUtils.CheckGLerror("GLES30.glUniformMatrix3fv");
	}
	*/
	...
	{
		int loc = GetUniformLocation(name);
		GLES30.glUniform1f(loc, val);
		BasicUtils.CheckGLerror("GLES30.glUniform1f");
	}

	void SetUniform(String name, int val)
	{
		int loc = GetUniformLocation(name);
		GLES30.glUniform1i(loc, val);
		BasicUtils.CheckGLerror("GLES30.glUniform1i");
	}
